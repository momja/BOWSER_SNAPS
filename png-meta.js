// png-meta.js — minimal PNG chunk utilities, no dependencies.
// Embeds a UTF-8 text payload as an iTXt chunk inserted just before IEND,
// and can read it back. Loaded via importScripts() in the service worker;
// the module.exports guard makes it requirable from Node for tests/tools.
const PngMeta = (() => {
  'use strict';
  ...
  function insertText(pngBytes, keyword, text) {
  ...
  function readText(pngBytes, keyword) {
    if (!isPng(pngBytes)) throw new Error('not a PNG');
    const decoder = new TextDecoder();
    let offset = 8;
    while (offset + 8 <= pngBytes.length) {
      const length = readU32(pngBytes, offset);
      if (chunkType(pngBytes, offset) === 'iTXt') {
  ...
  return { isPng, insertText, readText, toBase64, crc32 };
})();

if (typeof module !== 'undefined' && module.exports) module.exports = PngMeta;
