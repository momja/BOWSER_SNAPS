// collect.js — DOM metadata collection for a selected viewport region.
//
// Loaded twice, in two different worlds:
//   - MAIN world (before page-agent.js), where it can see framework internals
//     such as React fibers, Vue instances, and Angular debug hooks.
//   - ISOLATED world (before content.js), as a fallback when the page agent
//     is unavailable (e.g. the page loaded before the extension was installed).
//     There it still collects tags/ids/classes/attributes, just no component names.
var __bowserSnapsCollect = (() => {
  'use strict';
  ...
  // --- framework component detection (only works in the MAIN world) ---------
  ...
  // rect: { x, y, width, height } in CSS px, viewport-relative.
